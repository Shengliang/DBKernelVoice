...
export interface TopicCategory {
  category: string;
  icon: 'mysql' | 'postgres' | 'cloud' | 'rocksdb' | 'redis' | 'gpu' | 'case-study';
  topics: string[];
}

export const DATABASE_TOPICS: TopicCategory[] = [
  {
    category: "Cloud Native Case Studies",
    icon: 'cloud',
    topics: [
      "Aurora MySQL: The Log is the Database",
      "Aurora Storage: 6-way Replication & Quorums",
      "Aurora Internals: Hot Log vs Cold Log",
      "DynamoDB: Partitioning & Consistent Hashing",
      "DynamoDB: Paxos & Leader Election",
      "DynamoDB: Token Bucket Rate Limiting",
      "DynamoDB Global Tables: Conflict Resolution",
      "Google AlloyDB: Columnar Engine Internals",
      "AlloyDB: Intelligent Caching & Buffer Pool Extension",
      "AlloyDB: Vector Search Optimization",
      "AlloyDB: Cross-Region Replication Logic"
    ]
  },
  {
