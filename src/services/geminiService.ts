...
import { DetailedSearchResult, VoiceName, Language, HistoryItem, TopicResult } from "../types";
...
const CLIENT_VERSION = "6.12.0-DBKernel";
...
export const checkCacheExistence = (query: string, language: Language): DetailedSearchResult | null => {
...
export const getSavedHistory = (language: Language): HistoryItem[] => {
  ...
  searchCache.forEach((value, key) => {
    if (key.startsWith(`topic_db_v1_${language}_`)) {
      const result = value as DetailedSearchResult;
      history.push({
        key,
...
export const ensureInit = async () => {
    // This is a placeholder for the handshake logic if needed in future
    // Currently relying on direct API usage or local key
...
export const searchDBTopic = async (
  ...
  await ensureInit();
  const cacheKey = `topic_db_v1_${language}_${query.trim().toLowerCase()}_${lengthMinutes}`;
  
  if (!forceRefresh && searchCache.has(cacheKey)) {
    return searchCache.get(cacheKey) as DetailedSearchResult;
  }
  ...
  try {
    ...
    if (lengthMinutes >= 10) {
        detailInstructions = `
        CRITICAL: This is a LONG FORM DEEP DIVE (${lengthMinutes} minutes). 
        You MUST cover the following in extreme detail:
        1. **Data Structures**: Define the exact C/C++ structs used (e.g. in Raft, define the LogEntry, RequestVoteRPC).
        2. **Algorithms**: Step-by-step execution flow.
        3. **Edge Cases**: What happens during network partitions, disk failure, OOM?
        4. **Source Code**: Walk through specific files (e.g., 'raft_log.cc' or 'storage/innobase/handler').
        5. **Performance**: Locking strategies, latch contention, memory barriers.
        EXPAND the script significantly. Do not summarize.
        `;
    } else {
        detailInstructions = "Focus on the core concepts and a specific code example.";
    }
...
export const generateFollowUp = async (
    ...
    const prompt = `
    Context: A lecture on "${currentTopic}".
    User/Student Question: "${question}"

    Task: Continue the dialogue. The Student asks the question, and the Teacher answers it technically, referencing internals or code if applicable.
    
    Format:
    Student: [Question]
    Teacher: [Detailed Answer with C++ snippets if needed]
    ...
    return response.text || "Teacher: That is an interesting edge case...";
};

export const generateExtendedLecture = async (
    currentTopic: string,
    language: Language = 'en'
): Promise<string> => {
    await ensureInit();
    ...
    const prompt = `
    Context: A lecture on "${currentTopic}". The student wants to go deeper.

    Task: Continue the dialogue from where it left off. 
    1. Introduce more advanced concepts related to the topic (e.g., performance tuning, specific flags, or kernel implementation details).
    2. Provide another code snippet if relevant.
    3. Keep the "Teacher" and "Student" roleplay.

    Format:
    Student: Can you tell me more about the advanced implementation details?
    Teacher: Certainly. Let's look closer at...
    [Dialogue continues]
    ...
    return response.text || "Teacher: Let's continue deeper...";
};
