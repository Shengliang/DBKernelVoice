...
const App: React.FC = () => {
  ...
  const CLIENT_VERSION = "6.7.0-DBKernel";
  ...
  useEffect(() => {
    if (searchResult && scrollRefs.current[currentChunkIndex] && isPlayerOpen) {
      const activeEl = scrollRefs.current[currentChunkIndex];
      if (activeEl) {
        activeEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }
    }
  }, [currentChunkIndex, searchResult, isPlayerOpen]);

  // Auto-Parse Script when Search Result Loads
  useEffect(() => {
    if (searchResult?.passage.context) {
      const items = parseScriptToPlaylist(searchResult.passage.context);
      
      // If we have an overview, prepend it as the Introduction (System Role)
      if (searchResult.topic.overview) {
        const overviewChunks = splitTextIntoChunks(searchResult.topic.overview);
        const introItems: PlaylistItem[] = overviewChunks.map(chunk => ({
            text: chunk,
            role: 'System',
            voice: VoiceName.Charon // Different voice for Intro
        }));
        
        // Add intro items to the beginning
        setPlaylist([...introItems, ...items]);
      } else {
        setPlaylist(items);
      }

      // Pre-calculate title
      setCurrentAudioTitle(searchResult.passage.reference);
      // Reset audio cache for new topic
      audioCache.current = {};
  ...
  const resetAudioState = () => {
    ...
    // Don't clear playlist here, only when new search happens
    setCurrentChunkIndex(0);
    setIsPlayerOpen(false);
    setIsBuffering(false);
    setCurrentAudioTitle("");
    // Don't clear cache here to allow replaying
  };
  ...
  const executeSearch = async (query: string, forceRefresh: boolean = false) => {
    ...
    setError(null);
    fullReset(); // Clear everything for new search
  ...
  const handleStartLecture = () => {
      ...
      if (playlist.length === 0) {
          setError("No script to play. Please regenerate.");
          return;
      }
      playChunk(0);
  };
  ...
  const playChunk = async (index: number) => {
    ...
    try {
        const url = await fetchAudioChunk(index, playlist[index]);
        setAudioUrl(url);
        
        // Pre-fetch next chunk
        const nextIdx = index + 1;
        if (nextIdx < playlist.length) {
            fetchAudioChunk(nextIdx, playlist[nextIdx]);
        }
    } catch (err) {
  ...
  const getRoleLabel = (role: string) => {
    switch (role) {
      case 'Teacher': return 'Kernel Architect';
      case 'Student': return 'Junior DBA';
      case 'System': return 'Synopsis';
  ...
  const getRoleColor = (role: string) => {
    switch (role) {
      case 'Teacher': return 'text-tech-400';
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex text-stone-300 font-sans selection:bg-tech-900 selection:text-tech-100">
      ...
      <CacheModal
        ...
        onLoadCache={() => {
            if (pendingCacheData) {
                setSearchResult(pendingCacheData);
                setPendingCacheData(null);
            }
        }}
        ...
        <header className="sticky top-0 z-20 bg-[#050505]/95 backdrop-blur border-b border-stone-800">
          <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
             ...
             <div className="flex items-center gap-3 flex-wrap">
               ...
               <VoiceSelector selectedVoice={selectedVoice} onChange={setSelectedVoice} disabled={isPlayerOpen} />
             </div>
          </div>
        </header>
                  ...
                  <div className="bg-stone-900/50 p-6 flex flex-col md:flex-row justify-between items-start md:items-center border-b border-stone-800 gap-4">
                    ...
                    <div className="flex gap-2 w-full md:w-auto justify-center">
                      <button 
                        ...
                        disabled={isBuffering || playlist.length === 0} 
                        className="bg-tech-700 hover:bg-tech-600 text-white px-5 py-2 rounded flex items-center gap-2 shadow-lg shadow-tech-900/50 transition-all disabled:opacity-50 font-mono text-sm"
                      >
                         {isBuffering ? <Loader2 className="animate-spin w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
                        ...
                        {playlist.length > 0 ? (
                            playlist.map((item, idx) => (
                                <div 
                                    ...
                                    className={`transition-all duration-300 p-4 rounded border-l-4 cursor-pointer hover:bg-stone-900/80 ${
                                        idx === currentChunkIndex && isPlayerOpen
                                            ? 'bg-stone-900 border-tech-500 opacity-100 scale-100' 
                                            : 'bg-transparent border-transparent opacity-60 hover:opacity-100 scale-95'
                                    }`}
                                    title="Click to play from here"
                                >
                                    <div className="flex items-center gap-2 mb-2">
                                        <div className={`text-xs font-bold uppercase tracking-wider ${getRoleColor(item.role)}`}>
                                            {getRoleLabel(item.role)}
                                        </div>
                                        <Play className="w-3 h-3 text-stone-600 opacity-0 group-hover:opacity-100" />
                                    </div>
                                    <p className={`text-stone-300 whitespace-pre-wrap ${item.role === 'System' ? 'italic text-stone-400' : ''}`}>{item.text}</p>
      ...
      {isPlayerOpen && playlist.length > 0 && (
        <AudioPlayer 
          ...
          subtitle={
            playlist[currentChunkIndex]?.role === 'Teacher' ? 'Prof. Fenrir Speaking...' : 
            playlist[currentChunkIndex]?.role === 'Student' ? 'Student Asking...' : 
            'Introduction...'
          }
